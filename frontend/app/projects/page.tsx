  ...
  return (
    <div className="pt-32 pb-24 container mx-auto px-6 relative min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          <h1 className="text-4xl font-bold tracking-tight mb-2 text-foreground">Projects Radar</h1>
          <p className="text-foreground/50">Hop onto existing rockets or start your own.</p>
        </div>
        
        <div className="flex items-center gap-4 w-full md:w-auto">
          <Link href="/projects/new">
            <button className="px-5 py-3 bg-foreground text-background hover:bg-foreground/90 font-semibold rounded-full transition-all flex items-center gap-2 text-sm shadow-[0_0_20px_rgba(var(--shadow-rgb),0.2)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockProjects.map(proj => (
          <div key={proj.id} className="glass-card p-6 flex flex-col group relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-foreground mb-2 mt-2">{proj.title}</h3>
            <p className="text-sm text-foreground/60 mb-6 flex-1">{proj.desc}</p>
            
            <div className="mb-4">
              <p className="text-xs font-semibold text-foreground/40 uppercase mb-2">Roles Needed</p>
            ...
            <div className="flex items-center justify-between mt-auto pt-6 border-t border-foreground/5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-foreground/50">{proj.members}/{proj.size} filled</span>
              </div>
              <Link href={`/projects/${proj.id}`} className="text-sm font-semibold text-foreground hover:text-accent transition-colors flex items-center gap-1">
                View & Join &rarr;
              </Link>
            </div>
          </div>
